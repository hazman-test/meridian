...
// ═══════════════════════════════════════════
//  SECURITY INTEGRATION (TRAXR PRO)
// ═══════════════════════════════════════════
import TraxrModule from "./tools/traxr.js";
const traxr = new TraxrModule(process.env.TRAXR_API_KEY);

log("startup", "DLMM LP Agent starting...");
log("startup", `Mode: ${process.env.DRY_RUN === "true" ? "DRY RUN" : "LIVE"}`);
log("startup", `Model: ${process.env.LLM_MODEL || "hermes-3-405b"}`);

// Log security status to PM2/Console
if (traxr.isEnabled) {
    log("security", "🛡️ Traxr Pro: Security layer active.");
} else {
    log("security", "⚠️ Traxr Pro: API Key missing. Security fallback active.");
}
...
      
      // ─── SECURITY INTEGRATION: TRAXR PRO (CROSSWALK) ──────────────────
      const [smartWallets, narrative, tokenInfo, traxrProfile] = await Promise.allSettled([
        checkSmartWalletsOnPool({ pool_address: pool.pool }),
        mint ? getTokenNarrative({ mint }) : Promise.resolve(null),
        mint ? getTokenInfo({ query: mint }) : Promise.resolve(null),
        traxr.getPoolProfile(pool.pool) // REAL-TIME TRAXR SECURITY CALL
      ]);
      
      allCandidates.push({
        pool,
...
        txr: traxrProfile.status === "fulfilled" ? traxrProfile.value : null, 
        mem: recallForPool(pool.pool),
      });
      await new Promise(r => setTimeout(r, 150)); // avoid 429s
    }

    // Hard filters after token recon — block launchpads, excessive bots, and TRAXR security gate
    const passing = allCandidates.filter(({ pool, ti, txr }) => {
      // --- TRAXR SECURITY GATEKEEPER ---
      if (traxr.isEnabled && txr) {
        // PM2 Logging (visible via pm2 logs)
        log("security", `[TRAXR] Analysis for ${pool.name}: Score ${txr.score}/100 | Impact: ${txr.impact}`);

        if (txr.score < 75 || txr.impact === 'HIGH' || txr.impact === 'CRITICAL') {
          log("security", `❌ [REJECTED] ${pool.name} - Gatekeeper blocked due to high on-chain risk.`);
          
          // Telegram Notification
          if (telegramEnabled()) {
            sendMessage(`🛡️ **Traxr Security Alert**\nRejected pool: **${pool.name}**\nScore: ${txr.score}/100\nImpact: ${txr.impact}\n\n*Filtering out risky liquidity traps.*`).catch(() => {});
          }
          return false;
        }
      }
...
    if (passing.length === 0) {
      screenReport = `No candidates available (all blocked by safety filters).`;
      return screenReport;
    }
...
        txr ? `  security: Traxr Score=${txr.score}/100, Impact=${txr.impact}` : null,
